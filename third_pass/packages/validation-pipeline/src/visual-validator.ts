...
interface PixelComparisonResult {
  ...
  pixelDifferencePercent: number; // Percentage of different pixels
  pixelScore: number;             // 0-1 score (1 = identical)
  dimensions: {
    width: number;
    height: number;
  };
...
/**
 * Semantic comparison result (GPT-4o Vision)
 */
interface SemanticComparisonResult {
  ...
  semanticScore: number; // 0-1 score (1 = identical)
  latency: number;
  inputTokens?: number;
  outputTokens?: number;
  cost?: number;
 ...
 */
interface CombinedComparisonResult {
  success: boolean;
  pixelResult: PixelComparisonResult;
  semanticResult: SemanticComparisonResult;

  // Combined scores
  finalScore: number;     // Weighted combination (0-1, where 1 is identical)
  pixelWeight: number;    // Weight given to pixel score
  semanticWeight: number; // Weight given to semantic score
...
async function comparePixels(
  ...
  options: {
    ...
    saveDiffImage?: boolean;   // Save difference image
    diffImagePath?: string;    // Path to save diff image
  } = {}
...
export async function compareImages(
  ...
  options: {
    context?: string;
    pixelWeight?: number;      // Weight for pixel score (default 0.3)
    semanticWeight?: number;   // Weight for semantic score (default 0.7)
    saveDiffImage?: boolean;
    diffImagePath?: string;
    pixelThreshold?: number;
  } = {}
): Promise<CombinedComparisonResult> {
  const startTime = Date.now();
  ...
  const finalScore = (
    (pixelWeight * pixelResult.pixelScore) +
    (semanticWeight * semanticResult.semanticScore)
  );

  // Determine recommendation
  let recommendation: 'PASS' | 'NEEDS_REVIEW' | 'FAIL';
  let summary: string;
 ...
 */
export function formatComparisonResult(result: CombinedComparisonResult): string {
  const lines: string[] = [];
